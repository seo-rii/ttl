...
async function main() {
    ...
    const res = json.slice(1).map(row => {
        ...
        const title = row.__EMPTY_6;
        const group = (row.__EMPTY_5 || '').trim();
        const prof = row.__EMPTY_10;
        const where = row.__EMPTY_16;
